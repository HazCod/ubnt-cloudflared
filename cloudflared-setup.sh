...
if [ ! -f /etc/cloudflared/config.yml ] || [ "$1" = "pull" ]; then
	/usr/bin/curl -sf https://raw.githubusercontent.com/HazCod/ubnt-cloudflared/master/config.yml --output /etc/cloudflared/config.yml
fi
if [ ! -f /usr/local/bin/cloudflared ] || [ "$1" = "pull" ]; then
	sudo /usr/bin/curl -sf https://raw.githubusercontent.com/HazCod/ubnt-cloudflared/master/cloudflared --output /usr/local/bin/cloudflared
fi
